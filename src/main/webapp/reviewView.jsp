						...
						<div class="community-list-item">
							...
							<section class="CommunityListItem__StyledCommunityActionBar">
								<div class="review-like-area">
									<form action ="<%if (info != null && info.getMember_id() != null) {%>reviewViewLikeService<%} else {%>reviewView.jsp?reviewNum=<%=reviewNum%><%}%>">									
									<img src="img/like-icon.svg">
									<input name="reviewNum" value="<%=reviewNum%>" type="hidden">
									<input type="submit" value="좋아요">
									</form>
									<div><%=dbdao.getReviewLike(intReviewNum)%></div>
								</div>
								...
								<button class="like-button">
									...
									<div>조회수</div>
									<div><%=dbdao.getReviewSee(intReviewNum)%></div>
								</button>
							</section>
						</div>
